...
The default provider reads environment variables (``SMITHCORE_ASSET_*``);
...
DEFAULT_ASSET_PREFIX = "SMITHCORE_ASSET_"
...
class EnvAssetProvider:
    """Asset provider over ``SMITHCORE_ASSET_*`` environment variables.

    ``bot.asset("db.password")`` reads ``SMITHCORE_ASSET_DB_PASSWORD``:
...
def credential_fields(name: str, prefix: str = DEFAULT_ASSET_PREFIX) -> dict[str, str]:
    ...
    ``crm`` with ``login``/``password`` becomes ``SMITHCORE_ASSET_CRM_LOGIN``
    and ``SMITHCORE_ASSET_CRM_PASSWORD``. Keys are returned lower-cased
...
class HttpAssetProvider:
    """Fetch assets on demand from a smithcore-cloud orchestrator.
    ...
    ``SMITHCORE_ORCHESTRATOR_URL`` + ``SMITHCORE_AGENT_ID`` + a token; otherwise
...
def asset_provider_from_env() -> AssetProvider:
    ...
    The agent sets ``SMITHCORE_ORCHESTRATOR_URL`` / ``SMITHCORE_AGENT_ID`` /
    ``SMITHCORE_AGENT_TOKEN``; self-host runs use ``SMITHCORE_ASSET_*`` directly.
    """
    url = os.environ.get("SMITHCORE_ORCHESTRATOR_URL")
    agent_id = os.environ.get("SMITHCORE_AGENT_ID")
    token = os.environ.get("SMITHCORE_AGENT_TOKEN") or os.environ.get("SMITHCORE_API_TOKEN")
    process_id = os.environ.get("SMITHCORE_PROCESS_ID") or None
