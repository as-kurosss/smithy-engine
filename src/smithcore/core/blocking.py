...
which bounds the wait: default 30 s, tunable via the ``SMITHCORE_BLOCKING_TIMEOUT``
...
from smithcore.core.errors import PlatformError
...
def _default_timeout() -> float:
    raw = os.environ.get("SMITHCORE_BLOCKING_TIMEOUT", "30")
    try:
        value = float(raw)
    except ValueError:
        return 30.0
    ...
    with _executor_lock:
        if _executor is None:
            _executor = ThreadPoolExecutor(
                max_workers=1,
                thread_name_prefix="smithcore-blocking",
    ...
    **kwargs: Any,
    ...
    Raises:
        PlatformError: If the call does not finish within *timeout* seconds
            (default: ``SMITHCORE_BLOCKING_TIMEOUT`` env var, else 30 s).
    """
    ...
    try:
        ...
        raise PlatformError(
            ...
            "(tune SMITHCORE_BLOCKING_TIMEOUT if this is expected)",
        ) from exc
    finally:
        _exit_call()
