...
#: Framework-owned ``SMITHCORE_*`` settings that are consumed directly by
...
#: Prefixes of framework ``SMITHCORE_*`` namespaces that must never be
...
def _apply_env_overlay(document: dict[str, Any], prefix: str) -> None:
    ...
    ``SMITHCORE_ROBOT__QUEUE`` sets ``robot.queue``; double underscore nests,
    single underscores stay literal (``SMITHCORE_ROBOT_NAME`` → ``robot_name``).
    Values are typed with TOML scalar syntax (ints, bools, quoted strings).
    Framework-owned settings (``SMITHCORE_BLOCKING_TIMEOUT``,
    ``SMITHCORE_ALLOWED_COMMANDS``) are skipped — they configure the engine,
...
def load_config(
    ...
    *,
    ...
    env_prefix: str | None = "SMITHCORE_",
) -> Config:
    ...
    When *env_prefix* is set (default ``\"SMITHCORE_\"``), matching env vars
    override file values — per-environment tweaks without editing TOML.
    ``SMITHCORE_ROBOT__QUEUE`` sets ``robot.queue`` (``__`` nests, values are
