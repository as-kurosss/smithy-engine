...
Sandbox: when ``SMITHCORE_FILE_ROOT`` is set, every path must resolve
...
ENV_FILE_ROOT = "SMITHCORE_FILE_ROOT"
...
def confine_path(path: Path, *, env_var: str = ENV_FILE_ROOT) -> Path:
    ...
    default sandbox is ``SMITHCORE_FILE_ROOT``; screenshot output reuses
    this helper with ``SMITHCORE_OUTPUT_ROOT``.
    """
