"""HTTP queue backend speaking the smithcore-cloud transaction contract.
...
from smithcore.core.errors import InfrastructureError, InvalidInput
...
def _engine_version() -> str | None:
    """Installed smithcore-engine version (None when not resolvable).
    ...
    if _engine_version_cached is _ENGINE_VERSION_UNSET:
        try:
            _engine_version_cached = _pkg_version("smithcore-engine")
...
class HttpQueue:
    """Queue backend over the smithcore-cloud HTTP contract.

    *base_url* is the API root including the version/prefix path,
    e.g. ``"https://host/api"`` for a default smithcore-cloud deployment.
    ...
    def claim(self, queue: str, *, run_id: str, lease_seconds: int = 300) -> ClaimedItem | None:
        ...
        agent_version = os.environ.get("SMITHCORE_AGENT_VERSION")
        if agent_version:
