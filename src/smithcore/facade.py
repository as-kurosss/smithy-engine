...
from smithcore.core.assets import AssetProvider, EnvAssetProvider
from smithcore.core.errors import ElementNotFound, InvalidInput
from smithcore.core.events import EventBus, Middleware, ToolEvent
from smithcore.core.redact import redact_text, redact_value
...
class Smithcore:
    ...
    def __init__(
        ...
        *,
        ...
        if dev_capture is None:
            dev_capture = os.environ.get("SMITHCORE_DEV_CAPTURE", "").strip().lower() in (
                "1",
                "true",
                "yes",
            )
        ...
        if trace is not None:
            from smithcore.trace import FlowTracer
    ...
    def asset(self, name: str) -> str:
        ...
        default ``SMITHCORE_ASSET_*`` environment variables). Every value
    ...
    async def _keyed_config(self, key: str | None, base: dict[str, Any]) -> dict[str, Any]:
        ...
        if not self._dev_capture:
            raise InvalidInput(
                ...
                "dev capture (SMITHCORE_DEV_CAPTURE=1) to record it, or pass "
    ...
    async def screenshot(
        ...
        *,
        ...
        Requires ``mss`` and ``Pillow`` (included in ``smithcore[windows]``).
