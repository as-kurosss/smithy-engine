...
Usage:
  python -m smithcore.run_flow flow.json [--set NAME=VALUE ...] [--vars FILE] [--payload FILE]
  python -m smithcore.run_flow flow.json --tools my_tools.py
  python -m smithcore.run_flow flow.json --validate
  python -m smithcore.run_flow flow.json --transactional --queue NAME --db q.db
  python -m smithcore.run_flow flow.json --transactional --queue NAME --cloud URL --agent ID
...
Modes:
  ...
  --transactional        REFramework loop over a queue: each work item's
                         ...
                         --cloud/--agent (smithcore-cloud, token from the
                         SMITHCORE_CLOUD_TOKEN env var)
...
from smithcore.core.errors import BusinessError, InfrastructureError
...
def _load_tools(spec: str) -> list[Any]:
    ...
    if spec.endswith(".py"):
        module_spec = importlib.util.spec_from_file_location("smithcore_user_tools", spec)
        if module_spec is None or module_spec.loader is None:
            raise SystemExit(f"cannot import tools module: {spec}")
        module: Any = importlib.util.module_from_spec(module_spec)
        sys.modules["smithcore_user_tools"] = module
...
def _build_queue(args: argparse.Namespace) -> Any:
    ...
    if not args.cloud or not args.agent:
        message = (
            ...
            "or --cloud URL --agent ID (smithcore-cloud)"
        )
        raise SystemExit(message)
    from smithcore.core.http_queue import HttpQueue
...
def _seed_variables(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    :func:`smithcore.flow.parse_typed_value` so ``3`` can stay a number and
    ``"3"`` a string.
    """
...
async def _run_transactional(
    ...
    *,
    ...
    from smithcore.core.transactions import run_transactions_async
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="smithcore.run_flow", description=__doc__)
    parser.add_argument(
        "flow",
        nargs="?",
        default=None,
    ...
    parser.add_argument("--cloud", default=None, help="smithcore-cloud API base URL")
    parser.add_argument("--agent", default=None, help="agent id for the cloud queue")
    parser.add_argument(
        "--token-env", default="SMITHCORE_CLOUD_TOKEN", help="env var holding the queue token"
    )
    ...
    if args.pack:
        from smithcore.pack import load_manifest
